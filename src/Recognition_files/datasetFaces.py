...
class DatasetFaces:
    ...
    def __init__(self):
        ...
        self.cam.set(3, 640)
        self.cam.set(4, 480)
        self.face_detector = cv2.CascadeClassifier('Recognition_files//haarcascade_frontalface_default.xml')
    ...
    def start(self,id_student):
        ...
        while True:
            ...
            for (x,y,w,h) in faces:
                ...
                dummy_counter += 1
                cv2.imwrite("Recognition_files/dataset/Student." +
                            str(id_student) + '.' +
                            str(dummy_counter) + ".jpg", gray[y:y+h,x:x+w])
    ...
    def check_existence_directory(self):
        if not os.path.isdir("Recognition_files//dataset"):
            current_workspace = os.getcwd()
            path = os.path.join(current_workspace, "Recognition_files//dataset")
            os.mkdir(path)
